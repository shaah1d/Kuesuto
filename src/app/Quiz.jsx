...
[
  ...
  "questions": [
    ...
    const handleSubmitInput = (input) => {
        setIsLoading(true);
        const genAI = new GoogleGenerativeAI(process.env.NT_PUBLIC_API_KEY); // Replace with your actual API key
    ...
    const handleNextQuestion = (e) => {
        ...
        if (qnum < data.questions.length - 1) {
            ...
            if (qnum === data.questions.length - 2) {
                const genAI = new GoogleGenerativeAI(process.env.NT_PUBLIC_API_KEY);
                const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
